...
def test_merge_rejects_different_pass_evidence_instead_of_using_precedence():
    with pytest.raises(ReleaseReadinessProofMergeConflict, match="conflicting substantive"):
        merge_release_readiness_proof_bundles(
            _spec(),
            (
                _bundle(_proof("source.tests", ReleaseReadinessGateKind.SOURCE_VERIFICATION, reference="artifact://first")),
                _bundle(_proof("source.tests", ReleaseReadinessGateKind.SOURCE_VERIFICATION, reference="artifact://rerun")),
            ),
        )
...
def test_merge_rejects_pass_fail_conflict():
    with pytest.raises(ReleaseReadinessProofMergeConflict):
        merge_release_readiness_proof_bundles(
            _spec(),
            (
                _bundle(_proof("full.replace", ReleaseReadinessGateKind.FULL_REPLACE, reference="fabric-run://pass")),
                _bundle(_proof("full.replace", ReleaseReadinessGateKind.FULL_REPLACE, status=ReleaseReadinessStatus.FAIL, reference="fabric-run://fail")),
            ),
        )
...
def test_merge_requires_same_schema_and_exact_candidate_sha():
    ...
    with pytest.raises(ValueError, match="candidate git SHA mismatch"):
        merge_release_readiness_proof_bundles(_spec(), (_bundle(), _bundle(candidate="d" * 40)))
...
def test_merge_rejects_unknown_kind_drift_and_integration_backed_gate():
    with pytest.raises(ValueError, match="unknown gate"):
        merge_release_readiness_proof_bundles(
            _spec(),
            (_bundle(_proof("unknown.gate", ReleaseReadinessGateKind.FULL_REPLACE)),),
        )
    with pytest.raises(ValueError, match="kind mismatch"):
        merge_release_readiness_proof_bundles(
            _spec(),
            (_bundle(_proof("source.tests", ReleaseReadinessGateKind.FULL_REPLACE)),),
        )
    with pytest.raises(ValueError, match="integration-backed"):
        merge_release_readiness_proof_bundles(
            _spec(),
            (_bundle(_proof("fabric.pipeline", ReleaseReadinessGateKind.FABRIC_PIPELINE)),),
        )
...
def test_merge_rejects_credential_like_retained_reference_and_detail():
    with pytest.raises(ValueError, match="credential material"):
        merge_release_readiness_proof_bundles(
            _spec(),
            (_bundle(_proof("source.tests", ReleaseReadinessGateKind.SOURCE_VERIFICATION, reference="https://example.invalid/proof?access_token=secret")),),
        )
    with pytest.raises(ValueError, match="credential material"):
        merge_release_readiness_proof_bundles(
            _spec(),
            (_bundle(_proof("source.tests", ReleaseReadinessGateKind.SOURCE_VERIFICATION, detail="authorization: bearer redacted")),),
        )
...
def test_release_proofs_merge_cli_writes_exact_merged_bundle(tmp_path):
    ...
    first.write_text(json.dumps(_bundle(_proof("source.tests", ReleaseReadinessGateKind.SOURCE_VERIFICATION, reference="artifact://source-tests")).model_dump(mode="json")), encoding="utf-8")
    second.write_text(json.dumps(_bundle(_proof("full.replace", ReleaseReadinessGateKind.FULL_REPLACE, reference="fabric-run://full-replace")).model_dump(mode="json")), encoding="utf-8")
    exit_code = main(["release-proofs-merge", "--spec", str(spec), "--input", str(first), "--input", str(second), "--output", str(output)])
    assert exit_code == 0
...
def test_release_proofs_merge_cli_fails_closed_on_conflicting_reruns(tmp_path, capsys):
    ...
    for path, reference in ((first, "artifact://one"), (second, "artifact://two")):
        path.write_text(json.dumps(_bundle(_proof("source.tests", ReleaseReadinessGateKind.SOURCE_VERIFICATION, reference=reference)).model_dump(mode="json")), encoding="utf-8")
    exit_code = main(["release-proofs-merge", "--spec", str(spec), "--input", str(first), "--input", str(second), "--output", str(output)])
    assert exit_code == 2
