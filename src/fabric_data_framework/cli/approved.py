"""Approved environment/evidence CLI commands.

The command handlers here orchestrate reusable approved-runner contracts. They do
not own framework semantics; the reusable implementation stays outside ``cli``.
"""
...
def run_if_matched(argv: list[str]) -> int | None:
    """Run an approved/evidence command, returning ``None`` when it is not ours."""

    if argv and argv[0] == "integration-evidence-merge":
        return _run_merge(argv[1:])
    if argv and argv[0] == "integration-control-plane-certify-run":
        return _run_control_plane_certification(argv[1:])
    if argv and argv[0] == "integration-pipeline-run":
        return _run_pipeline(argv[1:])
    if argv and argv[0] == "integration-capture-run":
        return _run_capture(argv[1:])
    if argv and argv[0] == "integration-warehouse-run":
        return _run_warehouse(argv[1:])
    if argv and argv[0] == "integration-warehouse-fault-drill-run":
        return _run_warehouse_fault(argv[1:])
    return None


__all__ = ["APPROVED_COMMANDS", "run_if_matched"]
