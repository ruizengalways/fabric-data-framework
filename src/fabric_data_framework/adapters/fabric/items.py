...
class FabricItemCatalogClient(FabricRestClient):
    """Read-only Core Items API client with continuation-token pagination."""
    ...
    def list_items(
        ...
        *,
        ...
        Fabric's Core List Items endpoint is paginated. The client follows only the
        opaque continuation token while rebuilding the request against its configured
        Fabric API base URL; it does not follow an arbitrary continuation URI.
        """
        ...
        while True:
            ...
            payload, _ = self._request(
                "GET",
                f"workspaces/{workspace_id}/items{suffix}",
